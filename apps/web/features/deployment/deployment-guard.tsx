...
//
// ESTA ENTREGA SOLO DETECTA Y AVISA. Se compara la versión del bundle con la
// del servidor y, si difieren, aparece un cartel persistente: mejor un aviso
// que explica qué pasa que un botón girando sin respuesta.
//
// El bloqueo de los formularios queda para más adelante A PROPÓSITO. Devolver
// una acción vacía NO es una barrera segura: React puede dar el submit por
// completado y resetear los inputs no controlados, o sea perder lo tipeado —
// justo lo que este trabajo viene a evitar. Una barrera mal hecha es peor que
// el aviso solo.
//
...
/**
 * El estado del desfase, para quien necesite reaccionar a él.
 *
 * Hoy nadie lo consulta: el aviso lo pinta el propio guard. Queda expuesto como
 * la costura por donde va a entrar el bloqueo de mutaciones cuando exista una
 * barrera que no arriesgue perder lo tipeado.
 */
...
export function DeploymentGuard({ children }: { children: ReactNode }) {
  ...
  // Una vez detectado el desfase ya no hay nada que averiguar: la versión no va
  // a volver atrás. Seguir sondeando sería carga sin ninguna decisión detrás.
  ...
  useEffect(() => {
    ...
    // Se consulta YA, no dentro de un minuto. Una pestaña que se abre después
    // del despliegue tiene que enterarse ahora: esa ventana ciega es
    // exactamente el rato en que alguien opera sobre un build que ya no existe.
    void check();
    schedule();
