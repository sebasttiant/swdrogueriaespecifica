...
| Hash passwd | @node-rs/argon2       | 2.2.1          |
...
- Corepack (incluido en Node) para activar pnpm 12.4.1.
- Docker + Docker Compose (para levantar la base y la app).

### Bootstrap del toolchain (pnpm 12.4.1) — IMPORTANTE

Este proyecto **exige pnpm 12.4.1** (campo `packageManager`). Hay un detalle de
entorno a tener en cuenta:

> Si en tu máquina existe un pnpm global "standalone" más viejo que 12.4.1, ese
> pnpm intenta auto-provisionar 12.4.1 usando tu config **global** (p. ej.
...
```bash
# Opción A — un solo comando (recomendado): activa pnpm 12.4.1 + instala
...
```bash
./scripts/bootstrap.sh     # activa pnpm 12.4.1 + instala (ver "Bootstrap del toolchain")
...
> Si ya tenés pnpm 12.4.1 activo, podés usar `pnpm install` directamente en vez
> del bootstrap.
...
- **pnpm**: si en tu máquina hay un pnpm global más viejo que 12.4.1, su
